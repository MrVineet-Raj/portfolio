...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        res.data.message || "Error sending message. Please try again later."
      );
      setIsSubmitting(false);
      return;
    } catch (error) {
  ...
  return (
    <div className="w-full flex flex-col items-center border-b">
      <section id="contact" className="w-full max-w-[1400px]">
        <div className="container ">
          <div className="flex flex-col items-center text-center border-x border-b">
            <motion.div
              ...
              ref={ref}
              className="px-4 py-4 md:px-6 border-x"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2  items-start">
            <motion.div
              ...
              className=""
            >
              <Card className="rounded-none border-0 border-x  h-full">
            ...
            >
              <div className="pt-4">
                <h3 className="text-2xl font-bold px-4">Connect With Me</h3>
                <p className="text-muted-foreground mb-2 px-4">
                  You can also reach out to me through social media or schedule
                  a meeting directly.
                </p>
                <div className="grid grid-cols-2 border-t ">
                  {socialLinks.map((link, index) => (
                    <Button
                      key={index}
                      variant="outline"
                      className="w-full justify-start rounded-none border-0 border-r border-b"
                      asChild
                    >
                      <Link
                        href={link.url}
